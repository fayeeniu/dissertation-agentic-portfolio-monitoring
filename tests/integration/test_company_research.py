...
def test_company_number_runs_search_capture_exact_span_deck_and_named_review(
    ...
    try:
        ...
        with runtime.session_factory() as session:
            ...
            html = profile_deck_html(profile)
            assert "£2 million" in html
            assert "CBIT portfolio metric coverage" in html
            assert "Evidence-led investment dossier" in html
            assert "Related public signals" in html
            assert "Company input needed" in html
            assert "Not found in this run" in html
            assert "company-deck.css" not in html
        ...
        assert "investment advice" in deck.text
        assert "Potential contradictions requiring named resolution" in deck.text
        ...
        tampered_json = client.get(f"/profile-versions/{profile_id}/deck/json")
        tampered_html = client.get(f"/profile-versions/{profile_id}/deck/html")
        assert tampered_json.status_code == tampered_html.status_code == 422
        assert "unreviewed_mutation" not in tampered_json.text
    finally:
