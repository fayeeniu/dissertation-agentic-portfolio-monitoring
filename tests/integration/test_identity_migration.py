...
from portfolio_agent.models import CompanyModel, IdentityCandidateModel, ObservationModel
...
def test_name_only_import_requires_named_review(runtime: Runtime) -> None:
    result = runtime.importer.import_bytes(
        b"Metric,Name Only Synthetic\njobs_created,1\n",
        filename="name-only.csv",
        period_label="NAME-ONLY",
    )
    assert result.identity_hold_count == 1

    with runtime.session_factory.begin() as session:
        candidate = session.query(IdentityCandidateModel).one()
        imported = session.get(CompanyModel, candidate.imported_company_id)
        assert imported is not None
        assert imported.resolution_status == ResolutionStatus.UNRESOLVED.value
        decision = decide_identity_candidate(
            session,
            candidate_id=candidate.id,
            decision=IdentityDecisionType.ACCEPT,
            actor="Dissertation reviewer",
            reason="Validated against the authorised source record.",
        )
        assert decision.company_id == imported.id
        assert candidate.status == IdentityCandidateStatus.ACCEPTED.value
        assert imported.resolution_status == ResolutionStatus.RESOLVED.value
...
def test_identifier_collision_preserves_observations_under_identity_hold(runtime: Runtime) -> None:
    def submission(external_id: str, period: str) -> bytes:
        return json.dumps(
            {
                "schema_version": "1.0",
                "classification": "synthetic",
                "reporting_period": {"label": period},
                "companies": [
                    {
                        "external_id": external_id,
                        "name": "Collision Synthetic Ltd",
                        "metrics": {"jobs_created": 2},
                    }
                ],
            }
        ).encode()
    ...
    runtime.importer.import_bytes(
        submission("LEGACY-A", "COLLISION-A"),
        filename="collision-a.json",
        classification=DataClassification.SYNTHETIC,
    )
    held = runtime.importer.import_bytes(
        submission("LEGACY-B", "COLLISION-B"),
        filename="collision-b.json",
        classification=DataClassification.SYNTHETIC,
    )
        ...
        observations = list(
            session.scalars(
                select(ObservationModel).where(
                    ObservationModel.raw_submission_id == held.raw_submission_id
                )
            ).all()
        )
        candidate = session.scalar(
            select(IdentityCandidateModel).where(
                IdentityCandidateModel.raw_submission_id == held.raw_submission_id
            )
        )
    assert held.identity_hold_count == 1
    assert held.observation_count == 1
    assert len(observations) == 1
    assert candidate is not None
    assert observations[0].company_id == candidate.imported_company_id
