...
def test_only_unambiguous_inputs_resolve_through_catalogue() -> None:
    catalogue = MetricCatalogue()

    for row in CBIT_ROWS:
        resolved = catalogue.resolve(row.label)
        if row.role is CbitRowRole.INPUT:
            assert resolved is not None
            assert resolved.key == row.metric_key
            assert resolved.period_semantics is row.period_semantics
        else:
    ...
    assert all(metric.period_semantics is not PeriodSemantics.NONE for metric in catalogue.metrics)
...
def test_contract_canonicalisation_does_not_hide_alias_collisions() -> None:
    with pytest.raises(ValueError, match="Ambiguous metric alias"):
        MetricCatalogue(
            (
                MetricCatalogue().get("jobs_created"),
                MetricCatalogue()
                .get("awards_received")
                .model_copy(update={"aliases": ("Number of Jobs Created",)}),
            )
        )
