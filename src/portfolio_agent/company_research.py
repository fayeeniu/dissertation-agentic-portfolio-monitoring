...
class CompanyResearchService:
    ...
    def start(
        ...
        *,
        ...
        with self._session_factory.begin() as session:
            ...
            fingerprint = stable_hash(fingerprint_contract)
    ...
    def _validate_run_contract(
        ...
        persisted fields, so a run stays readable and its approved deck stays
        downloadable after the current prompt or policy version moves on. Set
        ``executing`` when a stage is about to run: the code that would execute it
        has changed, so the run must also be pinned to the current versions.
        """
        ...
        if restarted_from is not None:
            if not isinstance(restarted_from, str):
                raise CompanyResearchError(
                    "Research run restart lineage is invalid.", code="run_contract_tamper"
                )
            fingerprint_contract["restart_of_run_id"] = restarted_from
...
def validated_profile_content(
    ...
    if require_approved and profile.status != ProfileVersionStatus.APPROVED.value:
        raise CompanyResearchError("Only an approved company deck can be downloaded.")
    if stable_hash(profile.content_json) != profile.content_sha256:
        raise CompanyResearchError("Profile content hash does not match persisted content.")
...
def _presentation_report(content: Mapping[str, Any]) -> dict[str, Any]:
    """Rebuild presentation-only report metadata from the canonical claim ledger.

    This allows previously approved profiles to benefit from a clearer renderer
    without mutating their reviewed content or content hash.
    """

    claims: list[_PresentationClaim] = []
    for section in content.get("sections", []):
        if not isinstance(section, Mapping):
            continue
        category = str(section.get("key", "other"))
        for raw_claim in section.get("claims", []):
            if not isinstance(raw_claim, Mapping):
                continue
            claims.append(
                _PresentationClaim(
                    id=str(raw_claim.get("claim_id", "claim")),
                    category=category,
                    subject_key=str(raw_claim.get("subject_key", "other")),
                    statement=str(raw_claim.get("statement", "")),
                    evidence_span=str(raw_claim.get("evidence_span", "")),
                    source_locator=str(raw_claim.get("source_url", "")),
                    event_date=(
                        str(raw_claim["event_date"]) if raw_claim.get("event_date") else None
                    ),
                    amount=str(raw_claim["amount"]) if raw_claim.get("amount") else None,
                    currency=(
                        str(raw_claim["currency"]) if raw_claim.get("currency") else None
                    ),
                    perspective=str(raw_claim.get("perspective", "fact")),
                    verification_status=str(
                        raw_claim.get("verification_status", "verbatim_exact_span")
                    ),
                )
            )
    return build_investment_report(claims) if claims else dict(content.get("investment_report", {}))


def _safe_source_link(url: str) -> str:
    try:
        safe_url = canonical_public_url(url)
    except CompanyResearchError:
        return ""
...
def _evidence_card(claim: Mapping[str, Any], *, compact: bool = False) -> str:
    source_url = str(claim.get("source_url", ""))
    safe_url = _safe_source_link(source_url)
    metadata = []
    if claim.get("reported_value"):
        value = " ".join(
            item
            for item in (str(claim.get("currency") or ""), str(claim["reported_value"]))
            if item
        )
        metadata.append(f"<span>{html.escape(value)}</span>")
    if claim.get("event_date"):
        metadata.append(f"<span>{html.escape(str(claim['event_date']))}</span>")
    perspective = str(claim.get("perspective", "fact")).replace("_", " ").title()
    metadata.append(f"<span>{html.escape(perspective)}</span>")
    source = (
        f'<a href="{safe_url}" target="_blank" rel="noreferrer noopener">'
        f"{html.escape(_source_label(source_url))}</a>"
        if safe_url
        else "Source unavailable"
    )
    compact_class = " evidence-card--compact" if compact else ""
    return (
        f'<article class="evidence-card{compact_class}">'
        '<div class="evidence-card__meta">'
        + "".join(metadata)
        + "</div><blockquote>"
        + html.escape(str(claim.get("evidence_span") or claim.get("statement", "")))
        + '</blockquote><div class="evidence-card__source">'
        + source
        + " · exact captured span</div></article>"
    )


def _status_badge(status: str) -> str:
    labels = {
        "public_evidence": ("evidence", "Evidence found"),
        "document_required": ("document", "Company input needed"),
        "not_found_publicly": ("missing", "Not found in this run"),
        "definition_required": ("definition", "Definition needed"),
        "inputs_required": ("document", "Inputs needed"),
    }
    tone, label = labels.get(status, ("missing", status.replace("_", " ").title()))
    return f'<span class="status status--{tone}"><i aria-hidden="true"></i>{label}</span>'


def _metric_category_html(category: Mapping[str, Any]) -> str:
    signals = category.get("supporting_public_evidence", [])
    signal_html = ""
    if signals:
        cards = "".join(_evidence_card(item, compact=True) for item in signals[:4])
        overflow = len(signals) - 4
        signal_html = (
            '<aside class="signal-panel"><div><p class="eyebrow">Related public signals</p>'
            "<p>Useful context was found, but it does not by itself satisfy the exact metric, "
            "period, denominator or completeness requirement.</p></div>"
            f'<div class="signal-grid">{cards}</div>'
            + (
                '<p class="signal-overflow">'
                f"+ {overflow} additional signals in the evidence ledger</p>"
                if overflow > 0
                else ""
            )
            + "</aside>"
    ...
    for metric in category.get("metrics", []):
        ...
        evidence_html = (
            "".join(_evidence_card(item, compact=True) for item in evidence)
            if evidence
            else '<span class="empty-cell">No qualifying exact-span evidence</span>'
        )
        rows.append(
            "<tr><td data-label=\"Metric\"><strong>"
            + html.escape(str(metric.get("label", metric.get("key", "Metric"))))
            + '</strong><span class="metric-key">'
            + html.escape(str(metric.get("key", "")))
            + "</span></td><td data-label=\"Status\">"
            + _status_badge(str(metric.get("status", "unknown")))
            + "</td><td data-label=\"Period and boundary\"><span>"
            + html.escape(str(metric.get("period_semantics", "")).replace("_", " "))
            + '</span><span class="source-boundary">'
            + html.escape(str(metric.get("sourceability", "")).replace("_", " "))
            + "</span></td><td data-label=\"Evidence\">"
            + evidence_html
            + "</td></tr>"
        )
    return (
        '<section class="metric-group"><header><div><p class="eyebrow">CBIT metric family</p><h3>'
        + html.escape(str(category.get("key", "Metrics")))
        + "</h3></div><span>"
        + str(len(category.get("metrics", [])))
        + " metrics</span></header>"
        + signal_html
        + '<div class="table-shell"><table><thead><tr><th>Metric</th><th>Status</th>'
        + "<th>Period and boundary</th><th>Evidence</th></tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table></div></section>"
    )


def profile_deck_html(profile: ProfileVersionModel) -> str:
    content = validated_profile_content(profile)
    investment_report = _presentation_report(content)
    summary = investment_report.get("summary", {})
    claim_count = int(content.get("coverage", {}).get("claim_count", 0) or 0)
    source_count = int(content.get("coverage", {}).get("fetched", 0) or 0)
    coverage_percent = int(summary.get("public_metric_coverage_percent", 0) or 0)
    metric_categories = "".join(
        _metric_category_html(category) for category in investment_report.get("categories", [])
    )
    contradictions: list[str] = []
    for contradiction in content.get("contradictions", []):
        items = "".join(
            "<li><blockquote>"
            + html.escape(str(claim.get("evidence_span", "")))
            + '</blockquote><a href="'
            + html.escape(str(claim.get("source_url", "")), quote=True)
            + '">Source</a></li>'
            for claim in contradiction.get("claims", [])
        )
        contradictions.append(
            '<article class="contradiction"><h3>'
            + html.escape(str(contradiction.get("category", "claim")))
            + " · "
            + html.escape(str(contradiction.get("subject_key", "subject")))
            + "</h3><ul>"
            + items
            + "</ul></article>"
        )
    report_sections: list[str] = []
    for index, section in enumerate(investment_report.get("report_sections", []), start=1):
        key = str(section.get("key", "section"))
        if key == "source_and_methodology_appendix":
            continue
        evidence = section.get("evidence", [])
        cards = (
            "".join(_evidence_card(claim) for claim in evidence)
            if evidence
            else (
                '<div class="empty-state"><span aria-hidden="true"></span><div>'
                "<strong>No qualifying public evidence admitted</strong>"
                "<p>This is a visible diligence gap, not a zero value.</p></div></div>"
            )
        )
        report_sections.append(
            f'<section class="dossier-section" id="{html.escape(key, quote=True)}">'
            f'<header><span class="section-number">{index:02d}</span><div><h2>'
            + html.escape(str(section.get("heading", "Section")))
            + "</h2><p>"
            + html.escape(str(section.get("purpose", "")))
            + "</p></div><span class=\"section-count\">"
            + str(len(evidence))
            + " admitted</span></header><div class=\"evidence-grid\">"
            + cards
            + "</div></section>"
        )
    source_rows = "".join(
        ...
        + html.escape(
            str(
                source.get("publisher_domain")
                or _source_label(str(source.get("url", "")))
            )
        )
        + "</td><td>"
        + html.escape(str(source.get("source_tier", "public")).replace("_", " ").title())
        + "</td><td>"
        + html.escape(str(source.get("status", "unknown")).replace("_", " ").title())
        + "</td><td><a href=\""
        + _safe_source_link(str(source.get("url", "")))
        + '\" target="_blank" rel="noreferrer noopener">Open source</a></td></tr>'
        for source in content.get("sources", [])
        if _safe_source_link(str(source.get("url", "")))
    )
    review_label = str(profile.status).replace("_", " ").title()
    reviewed_by = profile.reviewed_by or "Named review pending"
    deck_styles = (
        Path(__file__).parent / "static" / "company-deck.css"
    ).read_text(encoding="utf-8")

    hero_html = (
        '<header class="report-hero"><div>'
        '<p class="eyebrow">Evidence-led investment dossier</p><h1>'
        + html.escape(str(content.get("company_name", "Company intelligence")))
        + '</h1><p class="lede">'
        "Public-source diligence organised for rapid human review. "
        + html.escape(str(investment_report.get("decision_boundary", "")))
        + '</p></div><aside class="review-card"><span class="approval">'
        + html.escape(review_label)
        + "</span><div><strong>Research contract</strong><dl>"
        "<div><dt>Companies House</dt><dd>"
        + html.escape(str(content.get("companies_house_number", "Not recorded")))
        + "</dd></div><div><dt>Evidence cutoff</dt><dd>"
        + html.escape(str(content.get("cutoff", "Not recorded")))
        + "</dd></div><div><dt>Model route</dt><dd>"
        + html.escape(str(content.get("model", "Not recorded")))
        + "</dd></div><div><dt>Review</dt><dd>"
        + html.escape(reviewed_by)
        + "</dd></div></dl></div></aside></header>"
    )
    ...
    coverage_html = (
        ...
        '<div class="coverage-panel__head"><div>'
        '<p class="eyebrow">Coverage at a glance</p>'
        '<h2 id="coverage-title">What the public record can support</h2>'
        "</div><p>"
        + str(coverage_percent)
        + "% of publicly searchable CBIT metrics have qualifying exact-span evidence."
        "</p></div>"
        f'<div class="coverage-track" role="img" aria-label="{coverage_percent}% '
        f'public metric coverage"><span style="width:{coverage_percent}%"></span></div>'
        '<div class="coverage-caption"><span>Bounded public search</span><span>'
        + str(summary.get("publicly_evidenced", 0))
        + " of "
        + str(summary.get("publicly_sourceable_metrics", 0))
        + ' eligible metrics</span></div><div class="stat-grid">'
        + stat(claim_count, "admitted evidence claims")
        + stat(source_count, "captured public sources")
        + stat(summary.get("publicly_evidenced", 0), "metrics directly evidenced")
        + stat(summary.get("supporting_public_signals", 0), "additional public signals")
        + stat(summary.get("document_required", 0), "company inputs required")
        + '</div><div class="legend" aria-label="Status legend">'
        + _status_badge("public_evidence")
        + _status_badge("document_required")
        + _status_badge("not_found_publicly")
        + "</div></section>"
    )
    contradictions_html = (
        '<section class="contradiction-wrap">'
        '<p class="eyebrow">Named resolution required</p>'
        "<h2>Potential contradictions requiring named resolution</h2>"
        + "".join(contradictions)
        + "</section>"
        if contradictions
        else ""
    )
    metric_appendix_html = (
        '<section class="metric-appendix"><header><div>'
        '<p class="eyebrow">Structured diligence appendix</p>'
        "<h2>CBIT portfolio metric coverage</h2></div><p>"
        "Green rows contain exact evidence for the canonical metric. Related public signals "
        "are shown separately and never treated as a completed metric. Amber rows require "
        "company documentation; grey rows were not found in this bounded search."
        "</p></header>"
        + metric_categories
        + "</section>"
    )
    sources_html = (
        '<section class="sources"><p class="eyebrow">Provenance appendix</p>'
        '<h2>Captured source register</h2><div class="table-shell"><table>'
        "<thead><tr><th>Publisher</th><th>Tier</th><th>Capture status</th>"
        "<th>Locator</th></tr></thead><tbody>"
        + source_rows
        + "</tbody></table></div></section>"
        if source_rows
        else ""
    ...
    limitations_html = (
        '<section class="limitations"><p class="eyebrow">Method boundary</p>'
        "<h2>Limitations</h2><ul>"
        + "".join(
            f"<li>{html.escape(str(item))}</li>" for item in content.get("limitations", [])
        )
        + "</ul></section>"
    )
    return (
