...
export function ReportsBoard({
  ...
  const nextAction = nextReview
    ...
    : {
        ...
        detail: "Profiles become exportable only after composition and a named decision.",
        href: "/companies",
      };
  ...
  return (
    <div className="overview">
      ...
      <p className="page-lede">
        {formatNumber(sources)} sources captured across versioned profiles. Export unlocks only after
        a named decision.
      </p>
          ...
          items={[
            {
              ...
              hint: review.length ? "Export stays locked until a named decision" : "No version is waiting",
            },
            {
              icon: "file-check",
              ...
              hint: "HTML and JSON unlock only after approval",
            },
            {
              icon: "versions",
              label: "Report versions",
              value: formatNumber(reportRuns.length),
              hint: "Immutable profile versions from composed runs",
            },
            {
              icon: "document",
            ...
            {reportRuns.length === 0 ? (
              ...
              <div className="report-library">
                {reportRuns.map((run) => {
                  ...
                  return (
                    <article className="report-card" key={profile.id} data-ready={canExport ? "true" : "false"}>
                      <div className="report-card-head">
                        ...
                        <span
                          ...
                          data-size="sm"
                          data-tone={canExport ? "evidence" : chartTone(profile.status)}
                      ...
                      <p className="muted" style={{ fontSize: "0.75rem" }}>
                        {canExport
                          ? "Named approval is on this exact version. The readable report and evidence package are unlocked."
                          : `${humanize(profile.status)}. Export stays locked until the named review gate is complete.`}
                      </p>
                      <div className="row">
                        <Link
                          className="btn"
                          data-variant={canExport ? undefined : "primary"}
                          data-size="sm"
                          href={`/runs/${run.id}`}
                        >
                          {canExport ? "Open run" : "Review version"}
                        </Link>
                        {canExport ? (
                          <>
                            <a
