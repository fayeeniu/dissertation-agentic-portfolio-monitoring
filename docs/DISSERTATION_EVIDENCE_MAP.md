...
| 2. Literature review | What is known about agentic workflows, LLM extraction, provenance, HITL, data quality, and portfolio reporting? | Architecture concepts and search themes only | No literature claims yet | Reproducible scholarly search, inclusion/exclusion, critical synthesis, page-cited notes |
...
| 4. Design and implementation | Why this architecture and how does it work? | Requirements, data dictionary, architecture, contracts, ADRs, source, tests, migration | P0 design and current implementation can be described | Final Git/artifact manifest, full validation output, known limitations |
...
| `alembic/versions/0001_initial_schema.py` | Reproducible persistence | Empty-database migration proof |
...
## Suggested dissertation figures and tables

| ID | Item | Source of truth | State |
|---|---|---|---|
| Fig 1 | Existing vs proposed reporting workflow | Transcript evidence + architecture; label stakeholder-described steps | Draftable |
| Fig 2 | Trust-boundary system context | `ARCHITECTURE.md` | Implemented |
| Fig 3 | Bounded agent state machine | `AGENT_CONTRACTS.md` | Implemented |
| Fig 4 | Experimental conditions and data partitions | `EVALUATION_PROTOCOL.md` | Protocol-only |
...
## Literature review evidence hold

The supplied materials are not a literature review. Before Chapter 2 is claim-ready, create a
reproducible search protocol across relevant scholarly databases for:

- multi-agent/agentic workflow evaluation and orchestration;
- LLM information extraction and structured outputs;
- hallucination/factuality, provenance, claim verification, and RAG evidence attribution;
- HITL decision support and automation bias;
- data quality/missingness/entity resolution in heterogeneous reporting;
- responsible AI/security/prompt injection; and
- impact/portfolio reporting in accelerators, venture capital, or innovation programmes.

Record query, database, date, filters, deduplication, inclusion/exclusion, and full-paper notes.
Use primary papers/standards for technical claims and retain page-level citations. Product pages
may establish capability/context, not academic effectiveness.
