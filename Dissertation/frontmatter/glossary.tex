...
CSRF & Cross-site request forgery: a browser attack in which another site makes requests on the user's behalf. \\
...
Held & \textit{(project term)} A verification outcome: the claim is not supported, is out of date or comes from untrusted content, so it is withheld from the report with its reason recorded. \\
HITL & Human in the loop: a person must approve an output before it can be released. \\
...
Nulls, missing states & \textit{(project term)} The eight distinct reasons a value can be absent, such as an explicit zero, not reported, or source unavailable, kept separate rather than merged into one blank. \\
OOS & Out of sample: data held back so it cannot influence how a system was built. \\
...
SSRF & Server-side request forgery: tricking a server into fetching an address it should not reach, such as an internal service. \\
