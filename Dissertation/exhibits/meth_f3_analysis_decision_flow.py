#!/usr/bin/env python3
"""Render METH-F3's accessible SVG as a deterministic vector PDF."""
...
try:
    ...
    raise SystemExit("ReportLab is required to render METH-F3") from exc
...
def parse_path(value: str) -> list[tuple[str, list[float]]]:
    """Parse the absolute M/L/Z paths deliberately used by METH-F3."""
...
def render(svg_path: Path, output_path: Path) -> None:
    ...
    pdf.setTitle("Analysis decision flow")
    pdf.setAuthor("Faye Niu")
    pdf.setSubject("METH-F3 conceptual quantitative and qualitative analysis decision flow")
