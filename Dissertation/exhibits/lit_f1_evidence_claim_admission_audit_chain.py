#!/usr/bin/env python3
"""Render LIT-F1's accessible SVG as a deterministic vector PDF."""
...
try:
    ...
    raise SystemExit("ReportLab is required to render LIT-F1") from exc
...
def declarations(value: str) -> dict[str, str]:
    """Parse the small CSS declaration subset used by the source SVG."""
    result: dict[str, str] = {}
...
def font_details(style: dict[str, str]) -> tuple[str, float]:
    """Map SVG font declarations to PDF core fonts for reproducibility."""
...
def element_style(element: ET.Element, classes: dict[str, dict[str, str]]) -> dict[str, str]:
    """Resolve class, inline and direct SVG presentation attributes."""
    style: dict[str, str] = {}
...
def text_segments(element: ET.Element, base: dict[str, str]) -> list[tuple[str, dict[str, str]]]:
    """Return text and optional tspan fragments in document order."""
...
def draw_text(pdf, element: ET.Element, height: float, classes: dict[str, dict[str, str]]) -> None:
    """Render one SVG text element without rasterisation."""
...
def parse_path(
    ...
    """Parse absolute M/L/C/Z paths and return the final segment vector."""
...
def draw_arrowhead(
    ...
    """Draw an orientation-aware triangular marker at the last path point."""
    dx = end[0] - previous[0]
    dy = end[1] - previous[1]
    ...
    pdf.setFillColor(colour(stroke_value, "#667784"))
    arrow = pdf.beginPath()
...
def draw_path(pdf, element: ET.Element, height: float, classes: dict[str, dict[str, str]]) -> None:
    """Render the source SVG's simple absolute paths and arrow markers."""
...
def render(svg_path: Path, output_path: Path) -> None:
    """Render one accessible SVG using only deterministic vector operations."""
    ...
    if style_node is None:
        raise SystemExit("SVG must retain its embedded style definitions")
    ...
    pdf.setTitle("Evidence-to-claim admission and audit chain")
    pdf.setAuthor("Faye Niu")
    pdf.setSubject("LIT-F1 conceptual evidence admission and audit chain")
    ...
    for element in root:
        ...
        if tag == "rect":
            ...
            width_value = number(element.get("width"))
            height_value = number(element.get("height"))
