#!/usr/bin/env python3
"""Render LIT-F2's accessible SVG as a deterministic vector PDF."""
...
try:
    ...
    raise SystemExit("ReportLab is required to render LIT-F2") from exc
...
def font_details(style: dict[str, str]) -> tuple[str, float]:
    """Map the SVG font declarations to deterministic PDF core fonts."""
...
def render(svg_path: Path, output_path: Path) -> None:
    ...
    pdf.setTitle("Discovery is not evidence")
    pdf.setAuthor("Faye Niu")
    pdf.setSubject("LIT-F2 conceptual discovery and evidence-boundary diagram")
