#!/usr/bin/env python3
"""Render SYS-F3's accessible SVG as a deterministic vector PDF."""
...
try:
    ...
    raise SystemExit("ReportLab is required to render SYS-F3") from exc
...
def render(svg_path: Path, output_path: Path) -> None:
    ...
    pdf.setTitle("Legal-identity intake and decision flow")
    pdf.setAuthor("Faye Niu")
    pdf.setSubject("SYS-F3 legal-identity intake, decision and downstream research gate")
