#!/usr/bin/env python3
"""Render SYS-F1's accessible SVG as a deterministic vector PDF."""
...
try:
    ...
    raise SystemExit("ReportLab is required to render SYS-F1") from exc
...
def set_stroke(pdf, style: dict[str, str], default: str = "#617684") -> None:
    """Apply stroke colour, width and optional dash pattern."""
    pdf.setStrokeColor(colour(style.get("stroke"), default))
    pdf.setLineWidth(number(style.get("stroke-width"), 1.0))
    dash_value = style.get("stroke-dasharray", "").strip()
    if dash_value and dash_value != "none":
        pdf.setDash([float(item) for item in NUMBER_RE.findall(dash_value)])
    else:
        pdf.setDash()
...
def render(svg_path: Path, output_path: Path) -> None:
    ...
    pdf.setTitle("Local architecture and deployment boundaries")
    pdf.setAuthor("Faye Niu")
    pdf.setSubject("SYS-F1 conceptual architecture and deployment status diagram")
